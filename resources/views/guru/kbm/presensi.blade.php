...
@section('content')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                 ...
                 <a href="/guru/kbm/detail/presensi/{{$kbm->id}}" class="btn btn-warning btn-sm">
    ...
    <script>
        ...
        $('#form-update-data').on('submit', function(e) {
            ...
            const url = "{{ route('g.kbm.presensi.update') }}";
            const method    = 'POST'; 
    ...
    <script>
        function tutupPresensiAjax() {
            $.ajax({
                url: "{{ route('g.kbm.presensi.tutup', $presensi->id) }}",
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        function aktifkan() {
                 Swal.fire({
                    ...
                    if (result.isConfirmed) {
                         $.ajax({
                            url: "{{ route('g.kbm.presensi.aktifkan', $presensi->id) }}",
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
